...
export type AppModalProps = CommonProps &
  ...
  PopItemProps & {
    ...
    result?: any;
    convertor?: (result: any) => any;
    asyncConvertor?: (result: any) => Promise<any>;
 ...
 * - `Z`: 表示 UTC 时间
 * - `8`: 表示 GMT+8 时区
 * - `auto`:  表示采用环境变量 `ENV_KEYS.TIMEZONE_OFFSET` 里的时区设置
 * - `null|undefined`: 表示采用用户浏览器本地时区设置
 */
export type DateParseOptionsZone = "Z" | "auto" | number;
