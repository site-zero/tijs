...
test("setTimeWithTimeZone+02:00", () => {
  let d = new Date("2025-12-05 02:00+8");
  let fmt = "yyyy-MM-dd HH:mm";
  expect(DateTime.format(d, { fmt, timezone: 2 })).eq("2025-12-04 20:00");
  DateTime.setTime(d, "04:00", 2);
  expect(DateTime.format(d, { fmt, timezone: 2 })).eq("2025-12-04 04:00");
  expect(DateTime.format(d, { fmt, timezone: 8 })).eq("2025-12-04 10:00");
});
...
test("setTime", function () {
  ...
  expect(DateTime.format(DateTime.setTime(d!, {}), { timezone: 8 })).eq(
    "2023-09-02 00:00:00"
  );
