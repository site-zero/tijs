...
const ShowRowMarker = computed(() => props.showCheckbox || props.showRowIndex);
  ...
  <div class="ti-table" :class="TopClass">
    <main ref="$main" :style="MainStyle">
      ...
      <template v-if="showHeader">
        ...
        <!-- 表头: 列 -->
        <div v-for="(col, i) in TableColumns" class="table-cell as-head"
          :col-index="i" :key="col.uniqKey" :col-key="col.uniqKey">
          <div class="head-cell-con">
            ...
            <!-- 调整列宽的控制柄: 最后一列 -->
            <div v-if="i == TableColumns.length - 1"
      ...
      <template v-for="row in TableData" :key="row.id">
        <!--================== < 表格行 > ================-->
        <div v-if="!isInRenderZone(row.index)" class="virtual-row"
          :style="VirtualRowStyle"></div>
        <TableRow v-else :columns="TableColumns" :showRowMarker="ShowRowMarker"
          :showCheckbox="showCheckbox" :showRowIndex="showRowIndex"
          :canHover="canHover" :canCheck="canCheck" :canSelect="canSelect"
          :row="row" :vars="vars" :activated="row.activated"
          :checked="row.checked" :indent="row.indent"
    ...
    <!-- 显示拖拽的列分割条-->
    <div class="resizing-bar" :style="ResizingBarStyle"
