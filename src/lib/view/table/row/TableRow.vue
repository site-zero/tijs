...
<template>
  <!--空白指示单元格-->
  <div v-if="props.showRowMarker" class="table-row-marker" :class="RowCellClass"
    :row-id="props.row.id" :row-index="props.row.index"
    @click="emit('select', { row: props.row, event: $event })">
    <!--选择框-->
    <template v-if="props.showCheckbox">
      <span class="as-checker"
    ...
    <!--行号-->
    <span v-if="props.showRowIndex" class="as-row-index">{{ props.row.index + 1
    }}</span>
    <!--Activated 指示器-->
    <span class="as-actived"
      @click="emit('open', { row: props.row, event: $event })"><i
        v-if="props.activated" class="zmdi zmdi-caret-right"></i></span>
  </div>
  <!--正式列单元格-->
  <template v-for="(cell, i) in props.columns" :key="cell.uniqKey">
    <div class="table-cell as-body" :row-id="props.row.id" :row-index="props.row.index"
      :col="i" :class="RowCellClass" ref="$cells">
      <!--首列，这里有可能插入缩进占位块-->
      <div v-if="0 === i && ShowIndentor" class="row-indent"
        :style="RowIndentStyle"></div>
      <!--插入单元格控件-->
      <TiCell v-bind="cell" :rowIndex="props.row.index" :colIndex="i"
        :data="props.row.rawData" :vars="props.vars"
        :activated="props.activated && i == props.activedColIndex"
        @click="emit('cell', { row: props.row, event: $event, colIndex: i })"
        @dblclick="emit('cell-open', { row: props.row, event: $event, colIndex: i })" />
    </div>
  </template>
</template>
<style lang="scss">
