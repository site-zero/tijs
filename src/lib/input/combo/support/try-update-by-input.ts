...
export async function try_update_by_input(api: InputComboApi, input: string) {
  ...
  if (debug) console.log(`try_update_by_input(input:[${input}])`);

  // 无需加载
  if (!api.hasOptionsData.value) {
    if (debug) console.log(`try_update_by_input(input:[${input}]) no options data`);
    return;
  }
  // 首先清理一下当前值
  ...
  if (api.shouldDelayTipReload.value) {
    if (debug)
      console.log(`try_update_by_input(input:[${input}]) debounce reload`);
    api.debounceTryReloadOptionsDataAndLookupItem(input);
  } else {
    if (debug)
      console.log(`try_update_by_input(input:[${input}]) immediate reload`);
