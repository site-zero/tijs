<script lang="ts" setup>
  ...
  import { BUS_KEY, TiAppBus, TiAppEvent, TiIcon } from '../../';
  import { computed, inject, onUnmounted, provide } from 'vue';
  import { BlockProps, COM_TYPE, useBlock, useBusAdaptor } from './use-block';
...
<template>
  <div class="ti-block" :class="Block.TopClass" :style="Block.TopStyle">
    <!--Block:Head-->
    <header v-if="Block.showHeadBar" :style="Block.HeadStyle">
      <TiIcon v-if="Block.BlockIcon" :value="Block.BlockIcon" />
      <div class="as-title" v-if="Block.BlockTitle">{{ Block.BlockTitle }}</div>
    </header>
    <!--Block:Main-->
    <main :style="Block.MainStyle">
      <component :is="Block.BlockComType" v-bind="Block.BlockComConf" />
    </main>
  </div>
</template>
