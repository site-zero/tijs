...
<template>
  <section class="ti-form-item" :type="FItem.race" :class="FItem.className"
    :style="TopStyle">
    <!--字段组-->
    <template v-if="'group' == FItem.race">
      ...
      <main :style="MainStyle" ref="$main">
        <TiFormItem v-for="it in FItem.fields" :key="it.uniqKey" v-bind="it"
          :data="FItem.data" :autoFieldNameMaxWidth="_auto_field_name_max_width"
    ...
    <!--普通字段-->
    <TiField v-else-if="FItem.props" v-bind="FItem.props" :data="FItem.data"
      :nameWidth="MaxFieldNameWidth" @change="emit('change', $event)" />
  </section>
</template>
<style lang="scss">
